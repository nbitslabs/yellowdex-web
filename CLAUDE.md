...
When changing or adding a page, manually update `public/sitemap.xml`, the `src/pages/llms.txt.ts` page registry, and the duplicated footer chrome. The top navigation is shared: `src/components/SiteHeader.astro` renders the primary nav (desktop links + mobile hamburger menu) for every page — pass `home` on `index.astro` so its section links stay in-page (`#features`) instead of routing home (`/#features`). Footers are still hand-duplicated per page and their link sets vary; footer links that also live in the header hamburger are hidden below `lg` (`hidden lg:inline`) so mobile doesn't show them twice. Keep `src/pages/privacy-policy.astro` and `src/pages/privacy-policy.md.ts` semantically identical.
...
The listing (`src/pages/blog/index.astro`), the dynamic `/blog/sitemap.xml` (referenced from `robots.txt`), and the Blog section of `src/pages/llms.txt.ts` **all derive from the collection**, so adding a post is self-contained: drop a valid `.mdx` file in `src/content/blog/` — no edits to `public/sitemap.xml`, the listing, or llms.txt required. Each post emits `BlogPosting` + `BreadcrumbList` JSON-LD; the listing emits `Blog` JSON-LD. The top nav comes from the shared `SiteHeader.astro`; footer chrome is still duplicated across pages (see Multi-surface sync).
